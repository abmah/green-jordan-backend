...
export const assignDailyChallengesService = async (userId) => {
  ...
  if (user.lastDailyAssignedAt && moment(user.lastDailyAssignedAt).isSame(today)) {
    return {
      ...
      dailyChallenges: user.dailyChallengesAssigned,
    };
  }
  ...
  if (availableChallenges.length < 2) {
    throw new Error('Not enough daily challenges available.');
  }
  ...
  return {
    ...
    dailyChallenges: availableChallenges,
  };
};
...
export const completeChallengeService = async (userId, challengeId) => {
  ...
  const user = await userModel.findById(userId)
    .populate('dailyChallengesAssigned');
  ...
  if (challengeCompleted) {
    throw new Error('Challenge has already been completed by the user.');
  }

  const assignedChallenge = user.dailyChallengesAssigned.find(c => c._id.equals(challengeId));
  if (!assignedChallenge) {
    throw new Error('Challenge is not part of the assigned daily challenges.');
  }
